...
#     'random_state': [22]
# }
# # Best Parameters: {'max_depth': 10, 'n_estimators': 100, 'random_state': 9}
# gs_cv = GridSearchCV(rfc, param_grid=param_dict, cv=2)
...
# 传入最优超参数, 重新进行训练
rfc = RandomForestClassifier(max_depth=10, n_estimators=100, random_state=9)
rfc.fit(x_train, y_train)

# 5. 模型评估
# 准确率
accuracy = dtc.score(x_test, y_test)
print("Cart Tree Accuracy:", accuracy)
rfc_acc = rfc.score(x_test, y_test)
print("Random Forest Accuracy:", rfc_acc)
# 指标报告
y_pred = dtc.predict(x_test)
report = classification_report(y_test, y_pred)
print("Cart Tree Report:", report)
...
# )
# plt.savefig("titanic_dtc.png", dpi=150)
# plt.show()